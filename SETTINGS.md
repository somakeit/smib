...
| SMIB_PLUGIN_SPACE_RELAY_STATE_RELAY_LIFETIME_ALERT_MESSAGE_TEMPLATE                | Slack alert message template. Supports `{device}`, `{relay_name}`, `{duration}` placeholders                                                                                                             | `{relay_name} needs replacing` | `:warning: {relay_name} on {device} has been active for {duration} since the last reset — it may need replacing.` |
