...
class SlackPluginIntegration:
    ...
    def disconnect_listeners(self, plugin: Plugin):
        self.logger.info(f"Locating slack listeners in {plugin.unique_name} ({plugin.name})")
        ...
        if isinstance(plugin, PythonModulePlugin) and hasattr(plugin._module, '__file__'):
            ...
            # For non-Python plugins, we would need a different way to identify listeners
            # This is a placeholder for future implementation
            self.logger.info(f"Non-Python plugin {plugin.unique_name} listeners disconnected")

    def disconnect_middlewares(self, plugin: Plugin):
        self.logger.info(f"Locating slack middleware in {plugin.unique_name} ({plugin.name})")
        ...
        if isinstance(plugin, PythonModulePlugin) and hasattr(plugin._module, '__file__'):
            ...
            # For non-Python plugins, we would need a different way to identify middlewares
            # This is a placeholder for future implementation
            self.logger.info(f"Non-Python plugin {plugin.unique_name} middlewares disconnected")
